...
import { getPendingReviews, approveReview, deleteReview } from "@/lib/reviewsDb";
...
// Pending reviews change whenever someone submits one — this can't be
// statically prerendered or new submissions won't show up until a redeploy.
    ...
    <div className="mx-auto max-w-3xl px-6 py-10">
      ...
      {pending.length === 0 ? (
        ...
        <ul className="mt-8 space-y-4">
          {pending.map((review) => {
            const villa = getVillaBySlug(review.villaSlug);
            const villaName = villa ? getVillaContent(villa, "en").name : review.villaSlug;

            return (
              <li key={review.id} className="rounded-2xl border border-border bg-surface p-5">
                <p className="text-xs font-semibold uppercase tracking-wide text-gold">
                  {villaName} · {review.locale.toUpperCase()} ·{" "}
